...
#+begin_src tidal :results silent
  ...
  d1 $ stack [
     ...
     , sound "sn:3*2 ~" -- ~は休符
     -- ハイハット
     , sound "~ hc ~ hc ~ hc ~ hc" # speed 0.8
   ]
    -- #でエフェクトをかける
    -- speed は音声サンプルの再生速度を変更できる
...
#+begin_src tidal :results silent
  d2
    $ sound "bass1:2*8"
    -- # up 2 -- 音程（ピッチ）を変更するエフェクト
    # up (choose [0, 2, 3, 0, 9]) -- choose関数で音程の高さをランダムにする  
    -- # legato 1 -- 音の長さを変更するエフェクト
    # legato (range 0.7 1 sine) -- range関数で音の長さをランダムにする
    # gain 0.8
#+end_src
...
org-modeとTidalCycles面白いです！暇な人触ってみてください。
 - 仕事ではあまり役に立たない技術だと思いますが暇つぶしとして面白いです。
